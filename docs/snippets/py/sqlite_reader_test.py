...
def test_sqlite_reader(tmp_path: Path):
    filename = Path(tmp_path) / 'test.db'
    with sqlite3.connect(filename) as db:
        db.execute('CREATE TABLE feature_table(hypothesis INT, feature1 FLOAT, feature2 FLOAT)')
        db.execute('INSERT INTO feature_table (hypothesis, feature1, feature2) VALUES (0, 1, 2), (0, 3, 4), (1, 5, 6)')
    db.close()
