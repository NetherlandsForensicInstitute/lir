...
class FeatureDataCsvParser(DataProvider, ABC):
    ...
    def __init__(
        ...
        :param head: read this number of rows from the file only, and ignore all others
    ...
    def _parse_file(self, fp: IO) -> FeatureData:
        ...
        for i, row in enumerate(reader):
            if self._head is not None and i == self._head:
                break
        ...
        if self._head is not None and len(features) < self._head:
            raise ValueError(f'input file has too few rows; expected: {self._head}; found: {len(features)}')
