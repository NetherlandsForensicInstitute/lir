...
def test_instance_pairing_seed():
    ...
    for values in zip(pairing0.pair(*data.get_instances()), pairing1.pair(*data.get_instances())):
        np.all(values[0] == values[1])

    pairing0 = InstancePairing(ratio_limit=1)
    pairing1 = InstancePairing(ratio_limit=1)
    for values in zip(pairing0.pair(*data.get_instances()), pairing0.pair(*data.get_instances())):
        np.all(values[0] == values[1])
    for values in zip(pairing0.pair(*data.get_instances()), pairing1.pair(*data.get_instances())):
        np.all(values[0] == values[1])


@pytest.mark.parametrize("pairing,n_pairs_found,features,labels,n_trace_instances,n_ref_instances", [
    (
...
def test_construct_array(pairing: np.ndarray, n_pairs_found: int, features: np.ndarray, labels: np.ndarray, n_trace_instances: int, n_ref_instances: int):
    meta = features
    ...
    assert n_pairs_found == pair_features.shape[0] == pair_labels.shape[0] == pair_meta.shape[0], "number of output pairs"
    assert pair_features.shape[1] == pair_meta.shape[1] == n_trace_instances + n_ref_instances
    assert pair_features.shape[2] == features.shape[1]
    assert pair_meta.shape[2] == meta.shape[1]
    assert np.all(pair_features.shape == np.array((n_pairs_found, n_trace_instances + n_ref_instances) + features.shape[1:]))


@pytest.mark.parametrize("n_pairs_expected,features,labels,n_trace_instances,n_ref_instances", [
    (
            3,
            np.ones((3, 1)),  # features
            np.arange(3),  # labels
...
def test_source_level_pairing(n_pairs_expected: int, features: np.ndarray, labels: np.ndarray, n_trace_instances: int, n_ref_instances: int):
    pair_features, pair_labels, pair_meta = SourcePairing().pair(features, labels, features, n_trace_instances, n_ref_instances)
    assert len(pair_labels) == n_pairs_expected
    assert np.all(pair_features.shape == np.array((n_pairs_expected, n_trace_instances + n_ref_instances) + features.shape[1:]))
