...
class TestFourParameterLogisticCalibrator(unittest.TestCase):
    ...
    def get_instances(self):
        ...
        return LLRData(features=X.reshape(-1, 1), hypothesis=y)
    ...
    def xtest_compare_to_logistic(self):
        ...
        logistic.fit(instances.features, instances.hypothesis)
    ...
    def test_pl_1_is_0(self):
        instances = self.get_instances()
        instances += LLRData(features=np.array([-np.inf, -np.inf]), hypothesis=np.array([0, 1]))
    ...
    def test_pl_0_is_1(self):
        ...
        instances += LLRData(features=probability_to_logodds(extra_instances_p), hypothesis=np.array([1, 1, 0, 0]))
    ...
    def test_pl_0_is_1_and_pl_1_is_0(self):
        ...
        instances += LLRData(
            features=probability_to_logodds(extra_instances_p), hypothesis=np.array([1, 1, 1, 1, 0, 0, 0, 0])
        )
