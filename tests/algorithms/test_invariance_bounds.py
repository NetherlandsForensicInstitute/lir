...
class UnboundLRs(DataProvider):
    ...
    def get_instances(self) -> LLRData:
        ...
        return LLRData(features=llrs, hypothesis=y)
...
class TestBounding(unittest.TestCase):
    ...
    def test_system01(self):
        ...
        bounds_bad = invariance_bounds.calculate_invariance_bounds(
            LLRData(features=odds_to_logodds(lrs), hypothesis=np.array([1, 1, 1, 0, 0]))
        )
        bounds_good1 = invariance_bounds.calculate_invariance_bounds(
            LLRData(features=odds_to_logodds(lrs), hypothesis=np.array([0, 0, 1, 1, 1]))
        )
        bounds_good2 = invariance_bounds.calculate_invariance_bounds(
            LLRData(features=odds_to_logodds(lrs), hypothesis=np.array([0, 0, 0, 1, 1]))
        )
