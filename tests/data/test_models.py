...
class BareData(InstanceData):
    def __len__(self) -> int:
        if self.hypothesis_labels is None:
            ...
            return self.hypothesis_labels.shape[0]


def test_instance_data():
    BareData(hypothesis_labels=None)
    BareData(hypothesis_labels=np.zeros((10,)))
    BareData(hypothesis_labels=np.zeros((10,)), meta=np.zeros((10,)))  # type: ignore
    BareData(hypothesis_labels=np.ones((10,)))
    BareData(hypothesis_labels=np.concatenate([np.zeros((10,)), np.ones((10,))]))

    # test all_fields property
    assert {'hypothesis_labels', 'source_ids'} == set(BareData(hypothesis_labels=None).all_fields)
    assert {'hypothesis_labels', 'source_ids', 'meta'} == set(BareData(hypothesis_labels=None, meta=1).all_fields)

    # test __eq__ method
    assert BareData(hypothesis_labels=np.ones((10,))) == BareData(hypothesis_labels=np.ones((10,)))
    assert BareData(hypothesis_labels=np.zeros((10,))) != BareData(hypothesis_labels=np.ones((10,)))
    assert BareData(hypothesis_labels=np.zeros((10,))) != BareData(hypothesis_labels=np.ones((10,)))
    assert BareData(hypothesis_labels=np.zeros((10,))) != BareData(hypothesis_labels=np.ones((10,)))
    assert BareData(hypothesis_labels=np.ones((10,)), meta=3) == BareData(hypothesis_labels=np.ones((10,)), meta=3)
    assert BareData(hypothesis_labels=np.ones((10,)), meta=2) != BareData(hypothesis_labels=np.ones((10,)), meta=3)
    assert BareData(hypothesis_labels=np.ones((10,)), meta=2) != BareData(hypothesis_labels=np.ones((10,)))

    # test slicing
    assert np.all(BareData(source_ids=np.arange(10))[:5].source_ids_1d == np.arange(5))
    assert np.all(BareData(hypothesis_labels=np.repeat([0, 1, 0, 1], 2))[:5].hypothesis_labels == np.array([0, 0, 1, 1, 0]))
    assert BareData(source_ids=np.arange(10))[8:9].source_ids_1d == np.array([8])

    # illegal labels type
    with pytest.raises(ValidationError):
        BareData(hypothesis_labels=1)  # type: ignore

    # illegal label dimensions
    with pytest.raises(ValidationError):
        BareData(hypothesis_labels=np.ones((10, 1)))

    # illegal operation
    with pytest.raises(ValidationError):
        instances = BareData(hypothesis_labels=np.array([0, 1]))
        instances.hypothesis_labels = np.array([1, 1])


def test_feature_data():
    FeatureData(features=np.ones((10, 2)), hypothesis_labels=None)
    FeatureData(features=np.ones((10, 2)), hypothesis_labels=np.ones((10,)))
    with pytest.raises(ValidationError):
        FeatureData(features=np.ones((10, 2)), hypothesis_labels=np.ones((11,)))

    # illegal operation
    with pytest.raises(ValidationError):
        instances = FeatureData(features=np.ones((10, 2)), hypothesis_labels=np.ones((10,)))
        instances.features = np.ones((10, 2))

    # initializing FeatureData with non-numeric feature values is an error
    with pytest.raises(ValidationError):
        FeatureData(features=np.array(['1'] * 10), hypothesis_labels=np.ones((10,)))
...
def test_sourceids():
    ...
    Check consistency and validation mechanism of `PairedFeatureData`.
    """
    FeatureData(features=np.ones((10, 2)), hypothesis_labels=np.ones(10), source_ids=np.ones((10, 1)))
    PairedFeatureData(
        features=np.ones((10, 2, 2)),
        hypothesis_labels=np.ones(10),
    ...
    with pytest.raises(ValueError):
        FeatureData(features=np.ones((10, 2)), hypothesis_labels=np.ones(10), source_ids=np.ones((11,)))
...
def test_llr_data():
    ...
    LLRData(features=np.ones((10, 1)), hypothesis_labels=np.ones(10))
    ...
    with pytest.raises(ValidationError):
        LLRData(features=np.ones((10,)), hypothesis_labels=np.ones(11))
...
def test_indexing():
    features = FeatureData(features=np.ones((10, 1)), hypothesis_labels=np.ones(10))
