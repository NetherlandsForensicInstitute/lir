...
class TestIsotonicRegression(unittest.TestCase):
    def test_lr_1(self):
        ...
        llrs = irc.fit_apply(LLRData(features=probability_to_logodds(X).reshape(-1, 1), hypothesis=y))
    ...
    def run_cllrmin(self, lr0, lr1, places=7):
        ...
        irc = IsotonicCalibrator()
        llrs = irc.fit_apply(LLRData(features=odds_to_probability(X).reshape(-1, 1), hypothesis=y))
    ...
    def test_lr_almost_1(self):
        ...
        X, y = Xn_to_Xy(score_class0, score_class1)
        irc = IsotonicCalibrator()
        llrs = irc.fit_apply(LLRData(features=X.reshape(-1, 1), hypothesis=y))
        lr0, lr1 = Xy_to_Xn(logodds_to_odds(llrs.llrs), y)
        self.assertEqual(score_class0.shape, lr0.shape)
        self.assertEqual(score_class1.shape, lr1.shape)
...
class TestLogitCalibrator(unittest.TestCase):
    ...
    def test_on_extreme_values(self):
        ...
        instances = LLRData(features=X.reshape(-1, 1), hypothesis=y)
