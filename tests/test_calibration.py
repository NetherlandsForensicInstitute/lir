...
from lir.util import Xn_to_Xy, Xy_to_Xn, odds_to_probability, probability_to_logodds, logodds_to_odds
...
class TestLogitCalibrator(unittest.TestCase):
    ...
    def test_prob_version(self):
        ...
        X = odds_to_probability(X)
        X = probability_to_logodds(X)
        desired = [0.1794121972972742, 0.00041466046383887233, 0.0002893188549697442, 0.00025706927831129677, 0.08740293107490102, 0.136624384190492, 0.0119387233899553, 2.5306657472748792e-05, 0.000585723303153669, 584.0144216917622, 72.58450757957488, 36.25831270280195, 30.37802498043253, 3.593416485014456, 24.124059461659918, 45.02979931421313, 73.83453694601398, 4744.539884048114]
    ...
    def test_on_extreme_values(self):
        ...
        X = probability_to_logodds(X)
        y = np.concatenate((np.zeros(12), np.ones(12)))
        desired = [0.17938619741047357, 0.0004145423988770249, 0.00028923408006262664, 0.0002569932531617985, 0.08738881700201688, 0.13660372753636074, 0.011936248051671832, 2.529782221300445e-05, 0.0005855611924232705, 0.0, 1.7898780873341496e-177, 0.0, 584.0381138155668, 72.58397448484948, 36.25750145609919, 30.377205489827194, 3.593143814999971, 24.12328565793142, 45.02902685404331, 73.83412361274337, 4751.481164586008, np.inf, 350076322.00372607, np.inf]
