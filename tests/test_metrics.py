...
class TestLR(unittest.TestCase):
    def test_calculate_cllr(self):
        ...
        self.assertAlmostEqual(2, metrics.cllr(*Xn_to_Xy([3.0] * 2, [1 / 3.0] * 2)))
        self.assertAlmostEqual(2, metrics.cllr(*Xn_to_Xy([3.0] * 20, [1 / 3.0] * 20)))
        self.assertAlmostEqual(
            0.4150374992788437, metrics.cllr(*Xn_to_Xy([1 / 3.0] * 2, [3.0] * 2))
        )
        self.assertAlmostEqual(
            0.7075187496394219, metrics.cllr(*Xn_to_Xy([1 / 3.0] * 2, [1]))
        )
        self.assertAlmostEqual(
            0.507177646488535, metrics.cllr(*Xn_to_Xy([1 / 100.0] * 100, [1]))
        )
        self.assertAlmostEqual(
            0.5400680236656377, metrics.cllr(*Xn_to_Xy([1 / 100.0] * 100 + [100], [1]))
        )
        self.assertAlmostEqual(
            0.5723134914863265,
            metrics.cllr(*Xn_to_Xy([1 / 100.0] * 100 + [100] * 2, [1])),
        )
        self.assertAlmostEqual(
            0.6952113122368764,
            metrics.cllr(*Xn_to_Xy([1 / 100.0] * 100 + [100] * 6, [1])),
        )
        self.assertAlmostEqual(1.0000000000000000, metrics.cllr(*Xn_to_Xy([1], [1])))
        self.assertAlmostEqual(
            1.0849625007211563, metrics.cllr(*Xn_to_Xy([2], [2] * 2))
        )
        self.assertAlmostEqual(
            1.6699250014423126, metrics.cllr(*Xn_to_Xy([8], [8] * 8))
        )
    ...
    def test_extreme_cllr(self):
        ...
        self.assertAlmostEqual(0.5, metrics.cllr(*Xn_to_Xy([0, 0], [1, 1])))
        self.assertAlmostEqual(0.5, metrics.cllr(*Xn_to_Xy([1, 1], [np.inf, np.inf])))
        self.assertAlmostEqual(0, metrics.cllr(*Xn_to_Xy([0, 0], [np.inf, np.inf])))
        self.assertAlmostEqual(
            np.inf, metrics.cllr(*Xn_to_Xy([np.inf, np.inf], [0, 0]))
        )
        self.assertEqual(
            np.inf, metrics.cllr(*Xn_to_Xy([1], [1.0e-317]))
        )  # value near zero for which 1/value causes an overflow
    ...
    def test_illegal_cllr(self):
        ...
        self.assertTrue(
            np.isnan(metrics.cllr(*Xn_to_Xy([np.nan, np.nan], [np.nan, np.nan])))
        )
...
class TestDevpavcalculator(unittest.TestCase):
    def test_devpavcalculator(self):
        ...
        PAVresult = np.array(
            [0.66666667, 0.66666667, 0.66666667, 0.66666667, 0.66666667, np.inf]
        )
        lrs, y = Xn_to_Xy(lrs_dif, lrs_same)
        ...
        lrs, y = Xn_to_Xy(lrs_dif, lrs_same)
        self.assertAlmostEqual(
            _devpavcalculator(lrs, fakePAVresult, y),
            (1 + 2 * (0.5 * 2 * 1 - 0.5 * 1 * 1) + 0.5) / 4,
        )
