...
from tests.data.datasets.lr_bounding import BoundingExample, UnboundLRs
...
class TestBounding(unittest.TestCase):
    ...
    def test_iv_paper_examples(self):
        ...
        llrs = UnboundLRs(example=BoundingExample.EXAMPLE_4).get_instances()
        ...
        llrs = UnboundLRs(example=BoundingExample.EXAMPLE_5).get_instances()
        bounds = invariance_bounds.calculate_invariance_bounds(logodds_to_odds(llrs.llrs), llrs.labels, llr_threshold)
