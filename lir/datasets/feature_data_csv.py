...
def _parse_feature_data_csv[ParserType: FeatureDataCsvParser](
    ...
    select_row_fn = _MatchIntPattern.parse(
        pop_field(config, 'select_rows', validate=partial(check_type, list), required=False)
    )

    parser = parser_class(**config, extra_fields=extra_fields, select_row_fn=select_row_fn, **kwargs)
    return parser
