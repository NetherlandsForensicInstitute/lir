...
class TestElub(unittest.TestCase):
    ...
    def test_extreme_smallset(self):
        ...
        y = np.array([1, 0])
        data = LLRData(features=odds_to_logodds(lrs), hypothesis=y)
    ...
    def test_system01(self):
        ...
        data_bad = LLRData(features=odds_to_logodds(lrs), hypothesis=np.array([1, 1, 1, 0, 0]))
        data_good1 = LLRData(features=odds_to_logodds(lrs), hypothesis=np.array([0, 0, 1, 1, 1]))
        data_good2 = LLRData(features=odds_to_logodds(lrs), hypothesis=np.array([0, 0, 0, 1, 1]))
    ...
    def test_bias(self):
        ...
        data1 = LLRData(features=odds_to_logodds(lrs1), hypothesis=y1)
        ...
        data2 = LLRData(features=odds_to_logodds(lrs2), hypothesis=y2)
        ...
        data3 = LLRData(features=odds_to_logodds(lrs3), hypothesis=y3)
    ...
    def test_bounded_calibrated_scorer(self):
        ...
        pipeline.fit(FeatureData(features=X, hypothesis=y))
