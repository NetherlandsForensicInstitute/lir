...
class TestElub(unittest.TestCase):
    ...
    def test_extreme_smallset(self):
        ...
        y = np.array([1, 0])
        data = LLRData(features=odds_to_logodds(lrs), hypothesis_labels=y)
    ...
    def test_system01(self):
        ...
        data_bad = LLRData(features=odds_to_logodds(lrs), hypothesis_labels=np.array([1, 1, 1, 0, 0]))
        data_good1 = LLRData(features=odds_to_logodds(lrs), hypothesis_labels=np.array([0, 0, 1, 1, 1]))
        data_good2 = LLRData(features=odds_to_logodds(lrs), hypothesis_labels=np.array([0, 0, 0, 1, 1]))
    ...
    def test_bias(self):
        ...
        data1 = LLRData(features=odds_to_logodds(lrs1), hypothesis_labels=y1)
        ...
        data2 = LLRData(features=odds_to_logodds(lrs2), hypothesis_labels=y2)
        ...
        data3 = LLRData(features=odds_to_logodds(lrs3), hypothesis_labels=y3)
    ...
    def test_bounded_calibrated_scorer(self):
        ...
        pipeline.fit(FeatureData(features=X, hypothesis_labels=y))
