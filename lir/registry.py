...
def _suggest_close_matches(key: str, candidates: Iterable[str], n: int = 3) -> list[str]:
    ...
    if len(key) >= 3:
        for c in candidates:
            if c != key and c.startswith(key) and c not in seen:
                seen.add(c)
                suggestions.append(c)
            if len(suggestions) >= n:
                return suggestions

    for c in difflib.get_close_matches(key, candidates, n=n, cutoff=0.6):
        if c not in seen:
            seen.add(c)
            suggestions.append(c)
        if len(suggestions) >= n:
...
class FederatedLoader(ConfigParserLoader):
    ...
    def get(
        ...
        for r in self.registries:
            ...
            try:
                for item in r:
                    candidates.add(item.split('.', 1)[1] if '.' in item else item)
            except Exception:
                LOG.debug(f'unable to enumerate candidates from {r}', exc_info=True)

        raise ComponentNotFoundError(
            '; '.join(str(e) for e in errors), key=key, candidates=candidates
        )
