...
def test_component_not_found_error_computes_suggestions_from_candidates():
    ...
    err = registry.ComponentNotFoundError('no such component', key='elub', candidates=['elub_bounder', 'iv_bounder'])
...
def test_fully_qualified_key_typo_gets_suggestion():
    """A typo in a fully qualified key (including the search path) should still yield a suggestion."""
    with pytest.raises(registry.ComponentNotFoundError) as exc_info:
        registry.get('data_providers.glas', search_path=['data_providers'])
    assert 'data_providers.glass' in exc_info.value.suggestions
    assert 'did you mean' in str(exc_info.value)


def test_nested_federated_loader_emits_suggestions_once():
    """A nested FederatedLoader must not add its own suggestions; the outermost emits them once."""
    import confidence

    yaml_registry = registry.YamlRegistry(confidence.Configuration({'a': {'foo': 'lir.registry'}}))
    nested = registry.FederatedLoader([yaml_registry, registry.ClassLoader()])
    loader = registry.FederatedLoader([nested])

    with pytest.raises(registry.ComponentNotFoundError) as exc_info:
        loader.get('fo', search_path=['a'])
    assert str(exc_info.value).count('did you mean') == 1
    assert exc_info.value.suggestions == ['foo']
