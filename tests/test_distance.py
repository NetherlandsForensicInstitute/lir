...
from lir.transform.distance import ManhattanDistance, ElementWiseDifference


def test_element_wise():
    dist = ElementWiseDifference()
    features = PairedFeatureData(features=np.stack([np.ones((10, 100)), np.ones((10, 100))*2], axis=1), n_trace_instances=1, n_ref_instances=1)
