...
class TestPairing(unittest.TestCase):
    ...
    def test_pairing1(self):
        ...
        self.assertEqual(np.sum(pairs.hypothesis == 1), 5, 'number of same source pairs')
        self.assertEqual(
            np.sum(pairs.hypothesis == 0),
            2 * (8 + 6 + 4 + 2),
    ...
    def test_pairing2(self):
        ...
        self.assertEqual(np.sum(pairs.hypothesis == 1), 5, 'number of same source pairs')
        self.assertEqual(np.sum(pairs.hypothesis == 0), 5, 'number of different source pairs')
    ...
    def do_test_pairing_ratio1(self, ratio_limit: float | int):
        ...
        ratio = np.sum(pairs.hypothesis == 0) / np.sum(pairs.hypothesis == 1)
    ...
    def test_pairing_ratio3(self):
        ...
        ratio = np.sum(pairs.hypothesis == 0) / np.sum(pairs.hypothesis == 1)
        self.assertEqual(ratio, ratio_limit, 'ratio ss ds pairs not correct')
        self.assertEqual(same_source_limit, np.sum(pairs.hypothesis == 1), 'ss pairs limit')
    ...
    def test_pairing_ratio4(self):
        ...
        self.assertLessEqual(np.sum(pairs.hypothesis == 0), different_source_limit, 'ds pairs limit')
    ...
    def test_pairing_ratio5(self):
        ...
        self.assertLessEqual(np.sum(pairs.hypothesis == 1), same_source_limit, 'ss pairs limit')
        self.assertLessEqual(np.sum(pairs.hypothesis == 0), different_source_limit, 'ds pairs limit')
