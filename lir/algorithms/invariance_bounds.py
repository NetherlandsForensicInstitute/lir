...
def plot_invariance_delta_functions(
    ...
    """
    llrs, y = llrdata.llrs, llrdata.hypothesis
    if y is None:
...
def calculate_invariance_bounds(
    ...
    sanitized_llrdata = LLRData(features=sanitized_llrs, hypothesis=y)
    ...
    lower_bound = float(min(lower_bound, 0.0))
    upper_bound = float(max(upper_bound, 0.0))
