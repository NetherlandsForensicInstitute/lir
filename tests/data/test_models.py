...
from lir.data.models import InstanceData, FeatureData, LLRData, concatenate_instances, PairData
...
def test_pair_data():
    PairData(features=np.ones((10, 9, 1)), n_trace_instances=4, n_ref_instances=5)

    with pytest.raises(ValueError):
        PairData(features=np.ones((10, 9)), n_trace_instances=4, n_ref_instances=5)

    with pytest.raises(ValueError):
        PairData(features=np.ones((10, 9, 1)), n_trace_instances=4, n_ref_instances=4)

    assert PairData(features=np.ones((10, 9, 1)), n_trace_instances=4, n_ref_instances=5).features_trace.shape == (10, 4, 1)
    assert PairData(features=np.ones((10, 9, 1)), n_trace_instances=4, n_ref_instances=5).features_ref.shape == (10, 5, 1)
    assert PairData(features=np.ones((10, 9, 3, 4)), n_trace_instances=4, n_ref_instances=5).features_ref.shape == (10, 5, 3, 4)
