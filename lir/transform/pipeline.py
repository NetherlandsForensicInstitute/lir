...
from lir.config.base import ConfigAttribute, YamlParseError, check_is_empty, config_parser, pop_field
...
def parse_steps(config: ConfigValue, output_dir: Path) -> list[tuple[str, Transformer]]:
    ...
    if config.value is None:
        return []
    module_names = list(
        config.as_dict(message=f'invalid value for "steps": expected `dict`; found: {type(config)}').keys()
    )
    return [
        (
            module_name,
...
class LoggingPipeline(Pipeline):
    ...
    def apply(self, instances: InstanceData) -> InstanceData:
        ...
        # add column: label
        if self.include_labels and instances.hypothesis is not None:
            csv_builder.add_column(instances.hypothesis, 'label')
...
def logging_pipeline(config: ConfigValue, output_dir: Path) -> Pipeline:
    ...
    return LoggingPipeline(steps, output_file, **config.as_dict())
