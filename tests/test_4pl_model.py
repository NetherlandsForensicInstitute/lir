...
class TestFourParameterLogisticCalibrator(unittest.TestCase):
    ...
    X_diff = read_data(
        os.path.join(
            dirname, "resources/LRsdifferentnormalLLRdistribmu_s=1N_ss=300.csv"
        )
    )
    X_same = read_data(
        os.path.join(dirname, "resources/LRssamenormalLLRdistribmu_s=1N_ss=300.csv")
    )
    ...
    def test_pl_1_is_0(self):
        ...
        np.testing.assert_almost_equal(
            devpav(logodds_to_odds(logodds), y), 0.12029952948152635, decimal=5
        )
    ...
    def test_pl_0_is_1(self):
        ...
        logodds = four_pl_model.transform(X)
        np.testing.assert_almost_equal(
            devpav(logodds_to_odds(logodds), y), 0.15273304557837525, decimal=5
        )
    ...
    def test_pl_0_is_1_and_pl_1_is_0(self):
        ...
        logodds = four_pl_model.transform(X)
        np.testing.assert_almost_equal(
            devpav(logodds_to_odds(logodds), y), 0.10475112893952891, decimal=5
        )
